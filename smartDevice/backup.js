	...
	$('#time_extend').click(function(){
		if(canClick){
			...
			if(type=="0"){
				...
				SendData(Encrypt('CONTROL ' + mac + ' P ' + passWord + ' AT+MSTIMESHUT=1?\r\n'));
			}
		}else{
			alert("该设备离线");
		}
	...
	$('#time_extend_ok').click(function(){
		...
		if(delay_long!=""){
			if(type=="0"){
				...
				SendData(Encrypt('CONTROL ' + mac + ' P ' + passWord + ' AT+MSTIMESHUT=1,' + delay_switch + ',' + delay_long + '\r\n'));
			}
		}
...
function addDeviceListControl(obj){
	$('.edit').css("display","none");
	deviceList=$('.apply_array .apply_img img');
	for(var i=0;i<deviceList.length;i++){
		deviceList[i].id="No" + i;
	}
	deviceList.click(function(){
	...
	for(var i=0;i<count;i++){
		switch(type_[i]){
			...
			case "W04":
				...
				img1.setAttribute("class","strip");
				var img2 = document.createElement("img");
				img2.setAttribute("src","imgs/plug.png");
				img2.setAttribute("class","strip");
				div_in.appendChild(img1);
				div_in.appendChild(img2);
